...
print (str((Path(__file__).parent / '..' / '..' / '..' / 'lib').resolve()))
...
from transnas_api.lib.models.task_models.decoder import FFDecoder
...
def get_cfg(encoder_str):
    ...
    # targets
    cfg['target_dim'] = 75  # (1024, 1024)
    cfg['target_load_fn'] = load_ops.load_class_object_logits
    cfg['target_load_kwargs'] = {
        'selected': True if cfg['target_dim'] < 1000 else False,
    ...
    cfg['warmup_epochs'] = 0 #TODO check if it's correct
